...
class Category_Management extends CI_Controller
{
	...
	public function index()
	{
		if (!empty($this->session->userdata('userid')) && $this->session->userdata('usr_logged_in') == 1) 
		{
			...
			$this->load->view('category/vw_categorylist', $data);
		} 
		else 
		{
	...
	public function onCreateCategoryView()
	{
		if (!empty($this->session->userdata('userid')) && $this->session->userdata('usr_logged_in') == 1) 
		{
			...
			$this->load->view('category/vw_create_category', $this->data, false);
		} 
		else 
		{
			redirect(base_admin_url());
	...
	public function onCreateCategory()
	{
		if (!empty($this->session->userdata('userid')) && $this->session->userdata('usr_logged_in') == 1) 
		{
			...
			$this->form_validation->set_rules('is_regional', 'Is Regional', 'trim|required|xss_clean|htmlentities');
			...
			{
				$category_name 	= xss_clean($this->input->post('category_name'));
				$is_regional 	= xss_clean($this->input->post('is_regional'));
				...
				redirect(base_admin_url('category-management'));		
			}	
		} 
		else 
		{
		...
		if (!empty($this->session->userdata('userid')) && $this->session->userdata('usr_logged_in') == 1) 
		{
			...
			if ($userdata) {
				$this->data['user_data'] = array(
													...
													'is_regional'  		=> $userdata->is_regional,
													'status'  			=> $userdata->status,
													'created_date'  	=> $userdata->created_date
												);
				$this->load->view('category/vw_edit_category', $this->data, false);
			} 
			else 
			{
				redirect(base_admin_url());
	...
	public function updateCategory()
	{
		if (!empty($this->session->userdata('userid')) && $this->session->userdata('usr_logged_in') == 1) 
		{
			...
			$is_regional 	= trim($this->input->post('is_regional'));

			$chkdata 		= array('id'  => $cat_id);
			$upd_userdata 	= array('category_name'  => $category_name,'is_regional' => $is_regional);
	...
	public function updateSubCategoryRegionalAjax()
	{
		if (!empty($this->session->userdata('userid')) && $this->session->userdata('usr_logged_in') == 1) 
		{
			if ($this->input->is_ajax_request() && $this->input->server('REQUEST_METHOD') == 'POST') 
			{
				...
				$chkdata 		= array('id'  			=> $category_id);
				$upd_userdata 	= array('is_regional'  	=> $is_regional);
